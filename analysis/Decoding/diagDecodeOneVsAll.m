function num_trials = diagDecodeOneVsAll(cfg0,subject)
...
elseif strcmp(cfg0.task,'localizer')
    ...
    stimIdx = 1; %index of trial info matrix that has stim classes 
else
...
%if ~exist(fullfile(outputDir,[cfg0.outputName '.mat']),'file')
    ...
    if strcmp(cfg0.task, 'detection')
        if cfg0.classifier < 3 
            %if house or face
            labels = data.trialinfo(:,miniBlockIdx)==cfg0.classifier & data.trialinfo(:,stimIdx) == 1; 
        elseif cfg0.classifier == 3
            labels = data.trialinfo(:,stimIdx)==2; 
        end
    end
    ...
    %idx = balance_trials(data.trialinfo(:,stimIdx),'downsample',y_binary=double(labels)+1); %idx is a Nclass x 1 cell, each cell with [Ntrials x 1] containing indices for trials of particular classs
