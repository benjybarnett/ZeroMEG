function [perf, result, testlabel] = mv_classify_timextime_BOB(cfg, X, clabel, X2, clabel2)

%BENJY'S EDITED SCRIPT ALLOWS CROSS DECODING TO BE USED WITH CROSS
        %VALIDATION
...
if ~strcmp(cfg.cv,'none') %will do cross validation even if there's a second datset for testing i.e. cross condition decoding
    ...
    % performed.
    if cfg.feedback, mv_print_classification_info(cfg,X,clabel); mv_print_classification_info(cfg,X2,clabel2);end
    ...
    for rr=1:cfg.repeat                 % ---- CV repetitions ----
        ...
        CV_train = mv_get_crossvalidation_folds(cfg.cv, clabel, cfg.k, cfg.stratify, cfg.p, cfg.fold);
        %create separate cv object for second datset
        CV_test = mv_get_crossvalidation_folds(cfg.cv, clabel2, cfg.k, cfg.stratify, cfg.p, cfg.fold);
        
        
        
        for kk=1:CV_train.NumTestSets                      % ---- CV folds ----
            if cfg.feedback
                if kk<=20, fprintf('%d ',kk), % print first 20 folds
                elseif kk==21, fprintf('... ') % then ... and stop to not spam the console too much
                elseif kk>CV_train.NumTestSets-5, fprintf('%d ',kk) % then the last 5 ones
                end
            end

            % Get train and test data
            [Xtrain, trainlabel,~, ~] = mv_select_train_and_test_data(X, clabel, CV_train.training(kk), CV_train.test(kk), 0);
            [~, ~, Xtest, testlabel{rr,kk}] = mv_select_train_and_test_data(X2, clabel2, CV_test.training(kk), CV_test.test(kk), 0);
